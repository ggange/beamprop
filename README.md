...
- **Laser-supported detonation** *(in progress)* — once lit, that plasma does not sit still: the absorption wave runs *back up the beam* toward the laser as a detonation. A 1-D Euler solver with laser deposition (M6c), coupled to the propagator as a pure absorber. The gas dynamics, the property table, and the physics gate are in; the propagator ↔ hydro outer loop and the CLI case are the remaining step.
...
| M6c | Laser-supported detonation wave ([docs/M6C_SPEC.md](docs/M6C_SPEC.md)), steps 1–5 of 6. Laser-agnostic 1-D Euler core (HLLC + MUSCL-Hancock, minmod, per-step CFL, positivity guard that bails rather than clamps), `IntensityScale` extracted from M4 (T4), frozen equilibrium plasma table to 30,000 K, and the coupled `LsdColumn` driver (Beer–Lambert attenuation, discretely conservative deposition, Strang-split source). **Verification:** Sod vs the exact Riemann solution, L1(ρ) 6.55e-3 → 6.55e-4 over n = 100 → 1600 at rate 0.79–0.88 (G1); observed order 1.86 → 1.94 on smooth flow (G2); coupled hydro↔source order 1.99/2.03/1.99 against a 1st-order contrast at 0.88/1.02/1.07 (G2b); Raizer's LSD velocity 5402 vs 5392 m/s, +0.19% (G3 — **verification, not validation**: it is the Chapman–Jouguet construction the model is built from); residual −8.26% → +0.19% as the absorption layer halves (G3b); seed-independent to 1.1e-3 (G3c); energy budget closed to 2.1e-16 (G5); plasma table vs direct Mutation++ off-grid, worst 1.48e-3 in `n_e` (G6). **Physics gate (G4):** `D ∝ S^(+0.33190)` over 1.52 decades and `ρ₀^(−0.33020)` over 1.50 decades, gated inside ±0.01 of ±1/3, with the level shown to move 59% under `γ` while the exponents move by 0.001. G7 (absolute velocity vs measurement) is documented and **ungated on purpose** — a planar solver has no radial relief. Remaining: the propagator ↔ hydro outer loop, the `lsd` CLI case, and the sub-cycling convergence check | **in progress (5/6 steps)** |
